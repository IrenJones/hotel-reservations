...
public class ReservationService {
    ...
    public List<ReservationDto> getByRoom(Long roomId) {
        return reservationRepository.findByRoomId(roomId).stream().map(ReservationService::toDto).toList();
    }
    ...
    public ReservationDto create(ReservationDto dto) {
        ...
        Reservation reservation = new Reservation(room, dto.guestName(), dto.guestEmail(),
                dto.checkInDate(), dto.checkOutDate(), status);
    ...
    private static ReservationDto toDto(Reservation r) {
        return new ReservationDto(r.getId(), r.getRoom().getId(), r.getGuestName(), r.getGuestEmail(),
